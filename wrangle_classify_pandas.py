...
for sic_chunk, company_chunk, sales_chunk, emp_chunk, misc_chunk in it.izip(sic_df, company_series, sales_df, emp_df,
    ...
    sic_chunk['Company'] = company_chunk
    sic_chunk.reset_index(inplace=True)
    sic_chunk.drop(['SIC2', 'SIC3', 'SIC4', 'SIC6', 'SIC8_2', 'SIC8_3', 'SIC8_4', 'SIC8_5', 'SIC8_6'], axis=1, inplace=True)
    sic_chunk.rename(columns={'SIC8': '8', 'SICChange':'Change'}, inplace=True)

    sic_chunk.columns = make_fullyear(list(sic_chunk.columns), 'SIC')
    ...
    misc_chunk['FirstYear'] = misc_chunk['FirstYear'] + 1
    misc_chunk.set_index(['DunsNumber', 'FirstYear'], inplace=True)

    joined = misc_chunk.join(nowlong, how='outer').ffill()
