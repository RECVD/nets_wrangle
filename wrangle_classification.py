...
class Classifier:
    ...
    def is_class(self, config_key, name, sic, emp, sales, tradename):
        ...
        try:
            sic_exclusive_2_bool = False
            sic_exclusive_2_bool= int(sic) in local_config['sic_exclusive_2']
        except KeyError:
            pass
        ...
        if np.isnan(name) or np.isnan(tradename):
        ...
        if condit_code == 1:
            ...
            if sic_exclusive_bool  or ((sic_range_bool or sic_exclusive_2_bool) and name_bool):
                return True
            else:
                return False
